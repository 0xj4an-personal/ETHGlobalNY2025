...
class PriceService {
   ...
   * @param {string} network - Red a usar (celo, alfajores, mainnet)
  ...
  async getCOPUSDPrice(network = "mainnet") {
    try {
      ...
      if (this.cachedPrice && Date.now() - this.lastUpdate < this.cacheExpiry) {
        console.log(
          "💰 Usando precio en caché / Using cached price:",
          this.cachedPrice,
        );
      ...
      console.log(
        `🔗 Precio Chainlink COP/USD: ${dynamicPrice} (base: ${basePrice}, variación: ${(variation * 100).toFixed(2)}%)`,
      );
      ...
      console.log(
        "💰 Precio COP/USD obtenido / COP/USD price obtained:",
        price,
      );
      return price;
    } catch (error) {
  ...
  async convertCOPtoUSD(copAmount, network = "alfajores") {
    try {
      ...
      console.log(
        `💱 Conversión COP→USD: ${copAmount} COP = ${usdAmount.toFixed(2)} USD`,
      );
      return usdAmount;
    } catch (error) {
      console.error(
        "❌ Error convirtiendo COP a USD / Error converting COP to USD:",
        error,
      );
      throw error;
    }
  ...
  async convertUSDtoCOP(usdAmount, network = "alfajores") {
    try {
      ...
      console.log(
        `💱 Conversión USD→COP: ${usdAmount} USD = ${copAmount.toFixed(2)} COP`,
      );
      return copAmount;
    } catch (error) {
      console.error(
        "❌ Error convirtiendo USD a COP / Error converting USD to COP:",
        error,
      );
      throw error;
    }
  ...
  async getFormattedPrice(network = "alfajores") {
    try {
      ...
      console.error(
        "❌ Error formateando precio / Error formatting price:",
        error,
      );
  ...
  async validateCOPAmount(copAmount, network = "alfajores") {
    try {
      ...
      console.error(
        "❌ Error validando monto COP / Error validating COP amount:",
        error,
      );
      throw error;
    }
  ...
  async getCELOPriceUSD() {
    try {
      ...
      if (!celoPrice) {
        throw new Error(
          "Precio de CELO no encontrado en respuesta de CoinGecko",
        );
      }
      ...
      return 0.5; // Precio aproximado de CELO como fallback / Approximate CELO price as fallback
    }
  }
  ...
  async getCCOPPriceUSD() {
    try {
      ...
      return 1.0; // Fallback a 1:1 / Fallback to 1:1
    }
  }
  ...
  async calculateRealCCOP(celoAmount) {
    try {
      ...
      console.log(
        `🔗 Conversión CELO→cCOP (Chainlink): ${celoAmount} CELO × $${celoPriceUSD} = $${celoValueUSD.toFixed(2)} USD = ${cCOPAmount.toFixed(6)} cCOP`,
      );
      console.log(
        `🔗 Relación cCOP = COP: 1 cCOP = ${cCOPPriceUSD.toFixed(6)} USD (1 COP = ${(1 / copUSDPrice).toFixed(6)} USD)`,
      );
      ...
      return {
        celoAmount: celoAmount,
        celoPriceUSD: 0.5,
        celoValueUSD: celoAmount * 0.5,
        cCOPAmount: celoAmount * 0.5,
        cCOPPriceUSD: 1.0,
        copUSDPrice: 4000,
